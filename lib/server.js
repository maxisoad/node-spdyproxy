var spdy = require('spdy'), //esta es de github
    http = require('http'), //esta es la de node
    path = require('path'),
    util = require('util'),
    net = require('net'),
    url = require('url'),
...
var SPDYProxy = function(options) {
  ...
  function handlePlain(req, res) {
    ...
    var dir = req.headers.host.split(':')[0]+req.url;
    console.log('URL: ' + dir.yellow);
    ...
    var sendRequest = function(){
      var rreq = http.request(requestOptions, function(rres) {
        ...
        //Headers
        //console.log('RRES HEADERS: ' + JSON.stringify(rres.headers));
        ...
        rres.on('end', function (){
          ...
          //self.base.newResource(dir,buffer,rres.headers);
    ...
    if(options.useCache){
      self.base.searchResource(dir,function(err,search){
        search.once('found',function(resultado){
          console.log('Proxy Cache HIT: '.blue,dir);
          //sendRequest();
          //console.log(resultado.resource.toString());
          //console.log(JSON.stringify(resultado.headers));
          //res.writeHead(200, '', resultado.headers);
          //res.end(resultado.resource.buffer,'binary');
          sendRequest();
        });
        search.once('notFound',function(resultado){
  ...
  function handleRequest(req, res) {
    //req: http.request -> http://nodejs.org/api/http.html#http_http_request_options_callback
    //res is the response
    console.log(req.method.magenta);
    var socket = (req.method == 'CONNECT') ? res : res.socket;

    if(typeof socket._spdyState === 'undefined') {
      console.log('WARNING: socket._spdyState is not defined'.yellow);
    }else{
        ...
        req.method, res.id || socket._spdyState.id,
        res.priority || socket._spdyState.priority
      );
    }
    ...
    //esta funcion me va a determinar si lo maneja con ssl (aca va con spdy) o plano
  ...
  self.base = new cache.createCache();
  if(options.useCache){
    self.base.startMongo();
    self.base.on('started',function(){
      console.log('MongoDB ready'.green);
      //use self porque el this aca tiene otro contexto
      spdy.server.Server.call(self, options);
      self.on("connect", handleRequest);
      self.on("request", handleRequest);
    });
  }else{
    spdy.server.Server.call(self, options);
    self.on("connect", handleRequest);
